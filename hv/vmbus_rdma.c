...
static void init_pfn(u64 *pfn, void *addr, u32 length)
{
	...
	for (i = 0; i < num_pfn; i++) {
		pfn[i] = (slow_virt_to_phys((u8*)addr + (PAGE_SIZE * i)) >> PAGE_SHIFT);
	}
...
void remove_uctx(struct hvnd_dev *nd_dev, struct hvnd_ucontext *uctx)
{
	...
	if (get_uctx(nd_dev, pid) == uctx)
		...
		idr_for_each_entry(&nd_dev->uctxidr, entry, id) {
			if (entry == uctx) {
		   ...
		   struct hvnd_cq *cq)
{
	...
	ret = hvnd_send_ioctl_pkt(nd_dev, &pkt->hdr, cq_pkt_size,
			       (u64)pkt);
			...
			union nd_sockaddr_inet *dest_addr, struct hvnd_ep_obj *ep)
{
	...
	pkt->ioctl.in.retry_cnt = 6;
	pkt->ioctl.in.rnr_retry_cnt = 0;
		   ...
		   struct hvnd_qp *qp)
{
	...
	ret = hvnd_send_ioctl_pkt(nd_dev, &pkt->hdr, qp_pkt_size,
			       (u64)pkt);
